...
def get_myPetsLower(name):
    ...
    cursor.execute(query)
    for (Name, id, Stage, Type, attr, pets, Equip_Slots, HP, XP, Atk, Def, level, moves, Image_link) in cursor:
        mypets[pets.lower()] = {"level": level, "Image_link": Image_link, "pets": pets, "equip": Equip_Slots, "hp": HP,
                        "xp": XP, "att": Atk, "def": Def, "stage": Stage, "attr": attr}
    cursor.close()
    cnx.close()
    return mypets
...
def get_level_l(p, n):
    pet = get_myPetsLower(n)
    level = pet[p]["level"]
    return str(level)


def get_curr_xp_l(p, n):
    pet = get_myPetsLower(n)
    currxp = pet[p]["xp"]
    return str(currxp)
...
def get_progress_xp_l(p, n):
    pet = get_myPetsLower(n)
    level = pet[p]["level"]
    progress = round(int(get_curr_xp(p,n))/int(get_max_xp(p, n, level)), 2)*100
    print(progress)
    return str(progress)


def get_max_xp_l(p, n, L):
    pet = get_myPetsLower(n)
    realm = round(L // 10)
    if realm == 0:
        realm = 1
...
def get_att_l(p, l, n):
    pet = get_myPetsLower(n)
    base_att = int(ap.get_att(p))/50 + pet[p]["att"]
    return str(base_att)
